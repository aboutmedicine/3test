...
THREE.OrbitControls = function ( object, domElement ) {
	...
	this.update = function () {
		...
		return function update() {
			...
			if ( scope.enableDamping ) {
				...
				var goalPositionIsAchieved = positionIsModified
					&& !(scope.object.position.x !== goalPosition.x
						|| scope.object.position.y !== goalPosition.y
						|| scope.object.position.z !== goalPosition.z);

				if (scope.target.x !== lookAt.x
					|| scope.target.y !== lookAt.y
					|| scope.target.z !== lookAt.z // lookAt is what we're going to look at. target is what we're looking at right now
				...
				{
					var targetDist = {
						x: lookAt.x - scope.target.x,
						y: lookAt.y - scope.target.y,
						z: lookAt.z - scope.target.z
					};
					...
					if (positionIsModified) {
						var targetPos = {
							x: goalPosition.x - scope.object.position.x,
							y: goalPosition.y - scope.object.position.y,
							z: goalPosition.z - scope.object.position.z
						};
	...
	function handleMouseDownPan( event ) {
        ...
        scope.dispatchEvent( {type: "panStart"} );
